...
public class ControleurListeUtilisateur implements Initializable {
    ...
    public void initialize(URL location, ResourceBundle resources) {
        ColonneMatriculeduTableauUtilisateur.setCellValueFactory(new PropertyValueFactory<Utilisateur, String>("Matricule"));
    ...
    private String generateCSVContent() {
        ...
        csvContent.append("Matricule, Nom, Prénom, Telephone, Adresse_Mail, groupe1, Type_utilisateur1, groupe2, Type_utilisateur2, groupe3, Type_utilisateur3, \n");
        // Parcourir la liste des groupes et ajouter les données au contenu CSV
        for (Utilisateur utilisateur : utilisateurArrayList) {
            csvContent.append(utilisateur.getMatricule()).append(",");
            csvContent.append(utilisateur.getNom()).append(",");
            csvContent.append(utilisateur.getPrenom()).append(",");
            csvContent.append(utilisateur.getTélephone()).append(",");
            csvContent.append(utilisateur.getAdresseMail()).append(",");
            csvContent.append(utilisateur.getGroupe1()).append(",");
            csvContent.append(utilisateur.getPrivilege1()).append(",");
            csvContent.append(utilisateur.getGroupe2()).append(",");
            csvContent.append(utilisateur.getPrivilege2()).append(",");
            csvContent.append(utilisateur.getGroupe3()).append(",");
            csvContent.append(utilisateur.getPrivilege3()).append("\n");
        ...
        if (file != null) {
            try {
                ...
                for (String line : lines) {
                    ...
                    if (columns.length >=7 ) {
                        String Matricule = columns[0].trim();
                        String Nom = columns[1].trim();
                        String Prenom = columns[3].trim();
                        String Telephone = columns[4].trim();
                        String  Email = columns[5].trim();
                       // Groupe Groupe1 = columns[6].trim();
                        String Type_utilisateur1 = columns[7].trim();

                        // Vérifier si les informations du groupe sont valides
                        boolean MatriculeValide = verificationmatricule(Matricule);
                       ...
                       // boolean Groupe1Valide = verificationGroupe(Groupe1);
                        boolean Priville1Valide = verificationprivillege(Type_utilisateur1);

                        if (MatriculeValide && NomValide && PrenomValide && TelephoneValide && EmailValide && Priville1Valide) {
                            // Créer un nouvel objet Groupe avec les informations du fichier CSV
                            Utilisateur utilisateur = new Utilisateur(Matricule,Nom,Prenom,Telephone,Email,Type_utilisateur1);
    ...
    void SupprimerDoublonUtilisateurAction(ActionEvent event) {
        ...
        for (Utilisateur utilisateur : utilisateurArrayList) {
            ...
            String groupeKey =utilisateur.getMatricule()  + utilisateur.getNom() + utilisateur.getPrenom() + utilisateur.getTélephone() + utilisateur.getAdresseMail() + utilisateur.getGroupe1() + utilisateur.getPrivilege1() + utilisateur.getGroupe2() + utilisateur.getPrivilege2() + utilisateur.getGroupe3() + utilisateur.getPrivilege3();
