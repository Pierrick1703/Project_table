...
public class ControleurGroupe implements Initializable {
    ...
    void lienlisteutilisateur(ActionEvent event) {
        try{
            Parent root = FXMLLoader.load(StartApplication.class.getResource("pageListeUtilisateur.fxml"));
            Scene scene = new Scene(root);
            Stage stage = (Stage) ((Node) event.getSource()).getScene().getWindow();
            stage.setScene(scene);
            stage.show();
    ...
    public void ExporterFicherGroupeAction(ActionEvent actionEvent) {
    }
}
